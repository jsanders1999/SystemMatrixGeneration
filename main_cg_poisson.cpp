...
int main(int argc, char* argv[])
{
	...
	{
	if (rank==p) std::cout << "Local grid on P"<<rank<<": [<<DD.nx_loc <<" x "<< DD.ny_loc <<" y "<<DD.nz_loc << "" <<std::endl; 
	//TODO do we want this print out?
	...
	*/
	...
	// Laplace operator
	stencil3d L = laplace3d_stencil(BP.bx_sz, BP.by_sz, BP.bz_sz);
	...
	for (int iz=0; iz<BP.bz_sz; iz++){
		double z = (BP.bz_idx+iz)*dz;
		for (int iy=0; iy<BP.by_sz; iy++){
			double y = (BP.by_idx+iy)*dy;
			for (int ix=0; ix<BP.bx_sz; ix++){
				double x = (BP.bx_idx+ix)*dx;
	...
	for (int iy=0; iy<BP.by_sz; iy++) {
		for (int ix=0; ix<BP.bx_sz; ix++){
			b[L.index_c(ix,iy,0)] -= L.value_b*g_0(ix*dx, iy*dy);
		}
	}

	// solve the linear system of equations using CG
	int numIter, maxIter=300;
